...
class DiceGame:
    def __init__(mainp, number_players, high_score):
        ...
        mainp.scores_record = []  # Record of scores and winners
    ...
    def play_game(mainp):
        ...
        # Print scores record
        print("Scores Record:")
        for i, (player, score) in enumerate(mainp.scores_record, start=1):
            print(f"Game {i}: Player {player} scored {score} points.")


# Example game type:
number_players = int(input("Enter number of players: "))
high_score = int(input("Enter target score to win: "))
game = DiceGame(number_players, high_score)
game.play_game()